...
const auth = {
  ...
  init() {
    ...
    $('#loginForm').addEventListener('submit', (e) => {
      ...
      const user = DB.data.auth.users.find(x => x.username === u && x.password === p);
      if (user) {
  ...
  showApp() {
    ...
    $('#userAvatar').textContent = (this.currentUser.name || 'A').charAt(0);
    router.go('dashboard');
  }
};
...
router.register('dashboard', () => {
  ...
  return `
    <div class="page-header">
      <h2>ภาพรวมระบบ</h2>
      <div class="actions">
        <button class="btn btn-primary" onclick="router.go('employees')">+ เพิ่มพนักงาน</button>
      </div>
    </div>
    <div class="stats-grid">
      <div class="stat-card">
        <div class="stat-icon bg-primary">👥</div>
        <div><div class="stat-label">พนักงานทั้งหมด</div><div class="stat-value">${fmt.num(s.totalEmployees)}</div><div class="stat-trend up">ปฏิบัติงาน ${s.activeEmployees} คน</div></div>
      </div>
      <div class="stat-card">
        <div class="stat-icon bg-blue">🏢</div>
        <div><div class="stat-label">จำนวนฝ่าย</div><div class="stat-value">${fmt.num(s.departments)}</div></div>
      </div>
      <div class="stat-card">
        <div class="stat-icon bg-green">💰</div>
        <div><div class="stat-label">เงินเดือนรวมต่อเดือน</div><div class="stat-value">${fmt.money(s.totalMonthlySalary)}</div></div>
      </div>
      <div class="stat-card">
        <div class="stat-icon bg-orange">🏦</div>
        <div><div class="stat-label">การกู้ที่ยังไม่ปิด</div><div class="stat-value">${fmt.num(s.activeLoans)}</div></div>
      </div>
      <div class="stat-card">
        <div class="stat-icon bg-purple">💵</div>
        <div><div class="stat-label">เบิกล่วงหน้ารอจ่าย</div><div class="stat-value">${fmt.num(s.pendingAdvances)}</div></div>
      </div>
      <div class="stat-card">
        <div class="stat-icon bg-red">📅</div>
        <div><div class="stat-label">วันหยุดในปฏิทิน</div><div class="stat-value">${fmt.num(DB.getCalendar().length)}</div></div>
      </div>
    </div>

    <div class="chart-row">
      <div class="chart-box">
        <div class="card-header"><div class="card-title">พนักงานตามฝ่าย</div></div>
        <canvas id="chartByDept"></canvas>
      </div>
      <div class="chart-box">
        <div class="card-header"><div class="card-title">สัดส่วนเพศ</div></div>
...
function renderDashboardCharts(s) {
  if (typeof Chart === 'undefined') {
    setTimeout(() => renderDashboardCharts(s), 200);
    return;
  }
  const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
  const textColor = isDark ? '#c9cfd6' : '#4b5563';
  ...
  if (ctx1) {
    new Chart(ctx1, {
      type: 'bar',
      data: {
        labels: s.byDepartment.map(d => d.name),
        datasets: [{
          label: 'จำนวน',
          data: s.byDepartment.map(d => d.count),
  ...
  if (ctx2) {
    new Chart(ctx2, {
      ...
      data: {
        labels: ['ชาย', 'หญิง'],
        datasets: [{ data: [s.byGender.male, s.byGender.female], backgroundColor: ['#2563eb', '#ec4899'] }]
      },
...
router.register = (origReg => function(name, fn) { origReg.call(this, name, fn); })(router.register);

const origGo = router.go;
router.go = function(name) {
  origGo.call(this, name);
  if (name === 'employees') {
    renderEmployeeList();
      ...
      <table class="table">
        <thead>
          <tr>
            ...
            <th>ฝ่าย</th><th>ตำแหน่ง</th>
            <th class="num">เงินเดือน</th><th>วันเริ่มงาน</th><th>สถานะ</th><th></th>
          </tr>
        </thead>
        <tbody>
          ${list.map(e => `
            <tr>
              ...
              <td class="actions">
                ...
                <button class="btn btn-ghost btn-sm" onclick="openEmployeeForm('${e.id}')">แก้ไข</button>
...
function openEmployeeForm(id = null) {
  const emp = id ? DB.getEmployee(id) : {
    ...
    phone: '', email: '', address: '',
    department: DB.getDepartments()[0]?.id || '',
    position: DB.getPositions()[0]?.id || '',
    ...
    <form id="empForm">
      <div class="form-grid">
        <div class="form-group">
          ...
          <input name="id" value="${escapeHtml(emp.id)}" required ${id ? 'readonly' : ''} />
        </div>
        <div class="form-group">
          <label>คำนำหน้า</label>
          <select name="title">
            ${['นาย', 'นาง', 'นางสาว', 'เด็กชาย', 'เด็กหญิง'].map(t => `<option ${emp.title === t ? 'selected' : ''}>${t}</option>`).join('')}
          </select>
        </div>
        <div class="form-group">
          <label>ชื่อ *</label>
          <input name="firstName" value="${escapeHtml(emp.firstName)}" required />
        </div>
        <div class="form-group">
          <label>นามสกุล *</label>
          <input name="lastName" value="${escapeHtml(emp.lastName)}" required />
        </div>
        <div class="form-group">
          <label>ชื่อเล่น</label>
          <input name="nickname" value="${escapeHtml(emp.nickname)}" />
        </div>
        <div class="form-group">
          <label>เลขบัตรประชาชน</label>
          <input name="nationalId" value="${escapeHtml(emp.nationalId)}" maxlength="13" />
        </div>
        <div class="form-group">
          <label>วันเกิด</label>
          <input name="dob" type="date" value="${emp.dob || ''}" />
        </div>
        <div class="form-group">
          <label>เพศ</label>
          <select name="gender">
            <option ${emp.gender === 'ชาย' ? 'selected' : ''}>ชาย</option>
            <option ${emp.gender === 'หญิง' ? 'selected' : ''}>หญิง</option>
          </select>
        </div>
        <div class="form-group">
          <label>เบอร์โทร</label>
          <input name="phone" value="${escapeHtml(emp.phone)}" />
        </div>
        <div class="form-group">
          <label>อีเมล</label>
          <input name="email" type="email" value="${escapeHtml(emp.email)}" />
        </div>
        <div class="form-group span-2">
          <label>ที่อยู่</label>
          <textarea name="address" rows="2">${escapeHtml(emp.address)}</textarea>
        </div>
        <div class="form-group">
          <label>ฝ่าย *</label>
          <select name="department" required>
            ${depts.map(d => `<option value="${d.id}" ${emp.department === d.id ? 'selected' : ''}>${escapeHtml(d.name)}</option>`).join('')}
          </select>
        </div>
        <div class="form-group">
          <label>ระดับตำแหน่ง *</label>
          <select name="position" required>
            ${positions.map(p => `<option value="${p.id}" ${emp.position === p.id ? 'selected' : ''}>${escapeHtml(p.name)}</option>`).join('')}
          </select>
        </div>
        <div class="form-group">
          <label>ชื่อตำแหน่ง</label>
          <input name="positionTitle" value="${escapeHtml(emp.positionTitle)}" placeholder="เช่น ผู้จัดการฝ่ายบุคคล" />
        </div>
        <div class="form-group">
          <label>วันเริ่มงาน *</label>
          <input name="hireDate" type="date" value="${emp.hireDate || ''}" required />
        </div>
        <div class="form-group">
          <label>เงินเดือน *</label>
          <input name="salary" type="number" min="0" step="100" value="${emp.salary || 0}" required />
        </div>
        <div class="form-group">
          <label>สถานะ</label>
          <select name="status">
            <option value="active" ${emp.status === 'active' ? 'selected' : ''}>ปฏิบัติงาน</option>
            <option value="resigned" ${emp.status === 'resigned' ? 'selected' : ''}>ลาออก</option>
          </select>
        </div>
        <div class="form-group span-2">
          <label>หมายเหตุ</label>
          <textarea name="note" rows="2">${escapeHtml(emp.note)}</textarea>
        </div>
      </div>
      <div class="form-actions">
        <button type="button" class="btn btn-secondary" data-close>ยกเลิก</button>
        <button type="submit" class="btn btn-primary">${id ? 'บันทึกการแก้ไข' : 'เพิ่มพนักงาน'}</button>
      </div>
    </form>`;
  modal.open(id ? 'แก้ไขข้อมูลพนักงาน' : 'เพิ่มพนักงานใหม่', html, { size: 'lg' });
  $('#empForm').addEventListener('submit', (e) => {
    e.preventDefault();
    const fd = new FormData(e.target);
    const data = Object.fromEntries(fd.entries());
    data.salary = Number(data.salary);
    DB.saveEmployee(data);
    modal.close();
...
async function deleteEmployee(id) {
  ...
  if (!emp) return;
  const ok = await modal.confirm('ลบพนักงาน', `ต้องการลบ ${emp.firstName} ${emp.lastName} ใช่หรือไม่?`);
  if (!ok) return;
  ...
  const renderTab = (tab) => {
    ...
    if (tab === 'history') {
      c.innerHTML = history.length ? `
        <div class="table-wrap"><table class="table"><thead><tr><th>วันที่</th><th>เงินเดือนเก่า</th><th>เงินเดือนใหม่</th><th>ส่วนต่าง</th><th>เหตุผล</th></tr></thead>
        <tbody>${history.map(h => `
          <tr><td>${fmt.date(h.date)}</td><td class="num">${fmt.money(h.oldSalary)}</td><td class="num">${fmt.money(h.newSalary)}</td>
          <td class="num">${fmt.money(h.newSalary - h.oldSalary)}</td><td>${escapeHtml(h.reason || '-')}</td></tr>`).join('')}</tbody></table></div>
      ` : '<div class="empty-state"><div class="hint">ยังไม่มีประวัติการปรับเงินเดือน</div></div>';
    } else if (tab === 'loans') {
      c.innerHTML = loans.length ? `
        <div class="table-wrap"><table class="table"><thead><tr><th>วันที่</th><th>จำนวน</th><th>ผ่อน/เดือน</th><th>คงเหลือ</th><th>สถานะ</th></tr></thead>
        <tbody>${loans.map(l => `<tr><td>${fmt.date(l.date)}</td><td class="num">${fmt.money(l.amount)}</td>
          <td class="num">${fmt.money(l.monthlyPayment)}</td><td class="num">${fmt.money(l.remaining)}</td>
          <td>${l.status === 'completed' ? '<span class="badge badge-success">ปิดยอด</span>' : '<span class="badge badge-warning">ผ่อนอยู่</span>'}</td></tr>`).join('')}</tbody></table></div>
      ` : '<div class="empty-state"><div class="hint">ยังไม่มีการกู้</div></div>';
    } else if (tab === 'advances') {
      c.innerHTML = advances.length ? `
        <div class="table-wrap"><table class="table"><thead><tr><th>วันที่</th><th>จำนวน</th><th>เหตุผล</th><th>สถานะ</th></tr></thead>
        <tbody>${advances.map(a => `<tr><td>${fmt.date(a.date)}</td><td class="num">${fmt.money(a.amount)}</td>
          <td>${escapeHtml(a.reason || '-')}</td>
          <td>${a.status === 'paid' ? '<span class="badge badge-success">จ่ายแล้ว</span>' : '<span class="badge badge-warning">รอจ่าย</span>'}</td></tr>`).join('')}</tbody></table></div>
      ` : '<div class="empty-state"><div class="hint">ยังไม่มีการเบิกล่วงหน้า</div></div>';
    } else if (tab === 'evals') {
      c.innerHTML = evals.length ? `
        <div class="table-wrap"><table class="table"><thead><tr><th>วันที่</th><th>คะแนน</th><th>เกรด</th><th>หมายเหตุ</th></tr></thead>
        <tbody>${evals.map(v => `<tr><td>${fmt.date(v.date)}</td><td class="num">${v.score}/100</td>
          <td><span class="badge badge-info">${escapeHtml(v.grade || '-')}</span></td><td>${escapeHtml(v.note || '-')}</td></tr>`).join('')}</tbody></table></div>
      ` : '<div class="empty-state"><div class="hint">ยังไม่มีการประเมิน</div></div>';
    }
  };

  renderTab('history');
  $$('#modalRoot .tab').forEach(t => {
...
function exportEmployeesXLSX() {
  if (typeof XLSX === 'undefined') {
    toast('กำลังโหลดไลบรารี Excel...', 'warning');
    setTimeout(exportEmployeesXLSX, 800);
    return;
  }
...
router.register('departments', () => {
  ...
  return `
    <div class="page-header">
      <h2>ฝ่าย / แผนก</h2>
      <div class="actions"><button class="btn btn-primary" onclick="openDeptForm()">+ เพิ่มฝ่าย</button></div>
    </div>
    <div class="card">
      <div class="table-wrap">
        <table class="table">
          ...
          <tbody>
            ${depts.map(d => {
              ...
              return `<tr>
                ...
                <td class="actions">
                  <button class="btn btn-ghost btn-sm" onclick="openDeptForm('${d.id}')">แก้ไข</button>
                  <button class="btn btn-ghost btn-sm" onclick="deleteDept('${d.id}')">ลบ</button>
                </td>
              </tr>`;
            }).join('')}
          </tbody>
  ...
  modal.open(id ? 'แก้ไขฝ่าย' : 'เพิ่มฝ่าย', `
    <form id="deptForm">
      <div class="form-grid">
        ...
        <div class="form-group span-2"><label>หัวหน้าฝ่าย</label>
          <select name="manager"><option value="">- ไม่ระบุ -</option>
            ${emps.map(e => `<option value="${e.id}" ${d.manager === e.id ? 'selected' : ''}>${escapeHtml(e.id + ' — ' + e.firstName + ' ' + e.lastName)}</option>`).join('')}
          </select>
        </div>
...
async function deleteDept(id) {
  ...
  const ok = await modal.confirm('ลบฝ่าย', `ต้องการลบฝ่าย "${d.name}" ใช่หรือไม่?`);
  if (!ok) return;
...
router.register('positions', () => {
  ...
  return `
    <div class="page-header">
      <h2>ระดับตำแหน่ง</h2>
      <div class="actions"><button class="btn btn-primary" onclick="openPositionForm()">+ เพิ่มระดับ</button></div>
    </div>
    <div class="card">
      <div class="table-wrap">
        <table class="table">
          ...
          <tbody>
            ${ps.map(p => `
              <tr>
                ...
                <td class="actions">
                  <button class="btn btn-ghost btn-sm" onclick="openPositionForm('${p.id}')">แก้ไข</button>
                  <button class="btn btn-ghost btn-sm" onclick="deletePosition('${p.id}')">ลบ</button>
                </td>
              </tr>`).join('')}
          </tbody>
        </table>
...
async function deletePosition(id) {
  ...
  const ok = await modal.confirm('ลบระดับ', `ต้องการลบระดับ "${p.name}" ใช่หรือไม่?`);
  if (!ok) return;
...
router.register('salary-adjust', () => {
  ...
  return `
    <div class="page-header">
      ...
      <div class="actions"><button class="btn btn-primary" onclick="openSalaryAdjustForm()">+ บันทึกการปรับ</button></div>
    </div>
    <div class="card">
      ...
      <div class="form-grid">
        <div class="form-group span-2">
          ...
          <select name="employeeId" id="adjEmp" required>
            ...
            ${emps.map(e => `<option value="${e.id}">${escapeHtml(e.id + ' — ' + e.firstName + ' ' + e.lastName + ' (' + fmt.money(e.salary) + ')')}</option>`).join('')}
          </select>
        </div>
        ...
        <div class="form-group"><label>ระดับตำแหน่งใหม่</label>
          <select name="newPosition"><option value="">- ไม่เปลี่ยน -</option>
...
router.register('loans', () => {
  ...
  return `
    <div class="page-header">
      ...
      <div class="actions"><button class="btn btn-primary" onclick="openLoanForm()">+ บันทึกการกู้</button></div>
    </div>
    <div class="card">
      ${loans.length ? `
      <div class="table-wrap">
          ...
          <tbody>
            ${loans.map(l => {
              ...
              return `<tr>
                ...
                <td class="actions">
                  <button class="btn btn-ghost btn-sm" onclick="openLoanForm('${l.id}')">แก้ไข</button>
                  <button class="btn btn-ghost btn-sm" onclick="deleteLoanRec('${l.id}')">ลบ</button>
                </td>
              </tr>`;
            }).join('')}
          </tbody>
...
function openLoanForm(id = null) {
    ...
    <form id="loanForm">
      <div class="form-grid">
        <div class="form-group span-2"><label>พนักงาน *</label>
          <select name="employeeId" required><option value="">- เลือก -</option>
            ${emps.map(e => `<option value="${e.id}" ${l.employeeId === e.id ? 'selected' : ''}>${escapeHtml(e.id + ' — ' + e.firstName + ' ' + e.lastName)}</option>`).join('')}
          </select>
        </div>
        ...
        <div class="form-group span-2"><label>สถานะ</label>
          <select name="status">
            <option value="active" ${l.status === 'active' ? 'selected' : ''}>ผ่อนอยู่</option>
            <option value="completed" ${l.status === 'completed' ? 'selected' : ''}>ปิดยอด</option>
          </select>
        </div>
  ...
  $('#loanForm').addEventListener('submit', (e) => {
    ...
    data.monthlyPayment = Number(data.monthlyPayment);
...
async function deleteLoanRec(id) {
  const ok = await modal.confirm('ลบรายการกู้', 'ยืนยันการลบ?');
  if (!ok) return;
  DB.deleteLoan(id); toast('ลบแล้ว', 'success'); router.go('loans');
}
...
router.register('advances', () => {
  ...
  return `
    <div class="page-header">
      ...
      <div class="actions"><button class="btn btn-primary" onclick="openAdvanceForm()">+ บันทึกการเบิก</button></div>
    </div>
    <div class="card">
      ${list.length ? `
      <div class="table-wrap">
        <table class="table">
          ...
          <tbody>
            ${list.map(a => {
              const e = DB.getEmployee(a.employeeId) || {};
              return `<tr>
                ...
                <td class="actions">
                  <button class="btn btn-ghost btn-sm" onclick="openAdvanceForm('${a.id}')">แก้ไข</button>
                  <button class="btn btn-ghost btn-sm" onclick="deleteAdvRec('${a.id}')">ลบ</button>
                </td>
              </tr>`;
            }).join('')}
          </tbody>
  ...
  modal.open(id ? 'แก้ไข' : 'บันทึกการเบิกล่วงหน้า', `
    <form id="advForm">
      <div class="form-grid">
        <div class="form-group span-2"><label>พนักงาน *</label>
          <select name="employeeId" required><option value="">- เลือก -</option>
            ${emps.map(e => `<option value="${e.id}" ${a.employeeId === e.id ? 'selected' : ''}>${escapeHtml(e.firstName + ' ' + e.lastName)}</option>`).join('')}
          </select>
        </div>
        ...
        <div class="form-group span-2"><label>สถานะ</label>
          <select name="status">
            <option value="pending" ${a.status === 'pending' ? 'selected' : ''}>รอจ่าย</option>
            <option value="paid" ${a.status === 'paid' ? 'selected' : ''}>จ่ายแล้ว</option>
          </select>
        </div>
...
async function deleteAdvRec(id) {
  const ok = await modal.confirm('ลบ', 'ยืนยัน?');
  if (!ok) return;
  DB.deleteAdvance(id); toast('ลบแล้ว', 'success'); router.go('advances');
}
...
router.register('allowance', () => {
  ...
  return `
    <div class="page-header">
      ...
      <div class="actions"><button class="btn btn-primary" onclick="openAllowanceForm()">+ บันทึก</button></div>
    </div>
    <div class="card">
      ${list.length ? `
      <div class="table-wrap">
          ...
          <tbody>
            ${list.map(a => {
              ...
              return `<tr>
                ...
                <td class="actions">
                  <button class="btn btn-ghost btn-sm" onclick="openAllowanceForm('${a.id}')">แก้ไข</button>
                  <button class="btn btn-ghost btn-sm" onclick="deleteAllowRec('${a.id}')">ลบ</button>
                </td>
              </tr>`;
            }).join('')}
          </tbody>
    ...
    <form id="allowForm">
      <div class="form-grid">
        ...
        <div class="form-group"><label>ประเภท</label>
          <select name="type">
            ${['ค่าเดินทาง', 'ค่าโทรศัพท์', 'ค่าตำแหน่ง', 'ค่าครองชีพ', 'อื่นๆ'].map(t => `<option ${a.type === t ? 'selected' : ''}>${t}</option>`).join('')}
          </select>
        </div>
...
async function deleteAllowRec(id) {
  const ok = await modal.confirm('ลบ', 'ยืนยัน?');
  if (!ok) return;
  DB.deleteAllowance(id); toast('ลบแล้ว', 'success'); router.go('allowance');
}
...
router.register('evaluations', () => {
  ...
  return `
    <div class="page-header">
      <h2>ประเมินผลงาน</h2>
      <div class="actions"><button class="btn btn-primary" onclick="openEvalForm()">+ บันทึกการประเมิน</button></div>
    </div>
    <div class="card">
      ${list.length ? `
      <div class="table-wrap">
        <table class="table">
          ...
          <tbody>
            ${list.map(v => {
              const e = DB.getEmployee(v.employeeId) || {};
              return `<tr>
                ...
                <td class="actions">
                  <button class="btn btn-ghost btn-sm" onclick="openEvalForm('${v.id}')">แก้ไข</button>
                  <button class="btn btn-ghost btn-sm" onclick="deleteEvalRec('${v.id}')">ลบ</button>
                </td>
              </tr>`;
            }).join('')}
          </tbody>
...
function openEvalForm(id = null) {
  ...
  modal.open(id ? 'แก้ไข' : 'บันทึกการประเมิน', `
    <form id="evalForm">
      <div class="form-grid">
        <div class="form-group span-2"><label>พนักงาน *</label>
          <select name="employeeId" required><option value="">- เลือก -</option>
            ${emps.map(e => `<option value="${e.id}" ${v.employeeId === e.id ? 'selected' : ''}>${escapeHtml(e.firstName + ' ' + e.lastName)}</option>`).join('')}
          </select>
        </div>
  ...
  const upd = () => { $('#gradeInput').value = scoreToGrade($('#scoreInput').value); };
  $('#scoreInput').addEventListener('input', upd);
  $('#evalForm').addEventListener('submit', (e) => {
    e.preventDefault();
    const data = Object.fromEntries(new FormData(e.target).entries());
    data.score = Number(data.score);
    if (id) data.id = id;
    DB.saveEvaluation(data);
    modal.close();
    toast('บันทึกแล้ว', 'success');
...
async function deleteEvalRec(id) {
  const ok = await modal.confirm('ลบ', 'ยืนยัน?');
  if (!ok) return;
  DB.deleteEvaluation(id); toast('ลบแล้ว', 'success'); router.go('evaluations');
}
        ...
        <table class="table">
          ...
          <tbody>
            ${DB.getDepartments().map(d => {
              ...
              return `<tr><td>${escapeHtml(d.name)}</td><td class="num">${list.length}</td>
                <td class="num">${fmt.money(sum)}</td>
                <td class="num">${fmt.money(list.length ? sum / list.length : 0)}</td></tr>`;
            }).join('')}
          </tbody>
        </table>
      </div>
...
function exportLoansXLSX() {
  ...
  const rows = DB.getLoans().map(l => {
    const e = DB.getEmployee(l.employeeId) || {};
    return {
      'วันที่': l.date, 'รหัสพนักงาน': l.employeeId,
      'ชื่อ-นามสกุล': (e.firstName || '') + ' ' + (e.lastName || ''),
...
router.register('calendar', () => {
  ...
  return `
    <div class="page-header">
      <h2>ปฏิทิน HR</h2>
      <div class="actions"><button class="btn btn-primary" onclick="openCalForm()">+ เพิ่มกิจกรรม</button></div>
    </div>
    ${upcoming.length ? `
    <div class="card">
      ...
      <div class="flex gap-3" style="flex-wrap:wrap">
        ${upcoming.map(c => `
          <div style="background:var(--surface-2);padding:12px 16px;border-radius:var(--radius);min-width:200px">
        ...
        <table class="table">
          ...
          <tbody>
            ${items.map(c => `<tr>
              ...
              <td class="actions">
                <button class="btn btn-ghost btn-sm" onclick="openCalForm('${c.id}')">แก้ไข</button>
  ...
  modal.open(id ? 'แก้ไข' : 'เพิ่มกิจกรรม / วันหยุด', `
    <form id="calForm">
      <div class="form-grid">
        ...
        <div class="form-group"><label>ประเภท</label>
          <select name="type">
            <option value="holiday" ${c.type === 'holiday' ? 'selected' : ''}>วันหยุด</option>
            <option value="event" ${c.type === 'event' ? 'selected' : ''}>กิจกรรม</option>
            <option value="other" ${c.type === 'other' ? 'selected' : ''}>อื่นๆ</option>
          </select>
        </div>
...
async function deleteCalRec(id) {
  const ok = await modal.confirm('ลบ', 'ยืนยัน?');
  if (!ok) return;
  DB.deleteCalendarItem(id); toast('ลบแล้ว', 'success'); router.go('calendar');
}
...
router.register('settings', () => {
  ...
  return `
    ...
    <div class="card">
      ...
      <form id="companyForm">
        <div class="form-grid">
          <div class="form-group span-2"><label>ชื่อบริษัท (ไทย)</label><input name="name" value="${escapeHtml(c.name)}"/></div>
          <div class="form-group span-2"><label>ชื่อบริษัท (อังกฤษ)</label><input name="nameEn" value="${escapeHtml(c.nameEn)}"/></div>
          <div class="form-group"><label>เลขทะเบียนภาษี</label><input name="taxId" value="${escapeHtml(c.taxId)}"/></div>
          <div class="form-group"><label>โทรศัพท์</label><input name="phone" value="${escapeHtml(c.phone)}"/></div>
          <div class="form-group span-2"><label>อีเมล</label><input name="email" value="${escapeHtml(c.email)}"/></div>
          <div class="form-group span-2"><label>ที่อยู่</label><textarea name="address" rows="2">${escapeHtml(c.address)}</textarea></div>
        </div>
        <div class="form-actions">
    ...
    <div class="card">
      ...
      <form id="pwForm">
        <div class="form-grid">
          <div class="form-group"><label>รหัสผ่านเก่า</label><input name="old" type="password" required/></div>
          <div class="form-group"><label>รหัสผ่านใหม่</label><input name="new" type="password" required minlength="6"/></div>
          <div class="form-group"><label>ยืนยันรหัสผ่านใหม่</label><input name="confirm" type="password" required minlength="6"/></div>
        </div>
        <div class="form-actions">
    ...
    <div class="card">
      ...
      <div class="flex gap-2" style="flex-wrap:wrap">
        <button class="btn btn-secondary" onclick="exportDataJSON()">📥 ดาวน์โหลดข้อมูลสำรอง</button>
        <button class="btn btn-secondary" onclick="document.getElementById('importFile').click()">📤 นำเข้าจาก backup</button>
        <input type="file" id="importFile" accept=".json" style="display:none" onchange="importBackup(event)"/>
        <button class="btn btn-danger" onclick="resetData()">⚠ รีเซ็ตข้อมูลทั้งหมด</button>
      </div>
      <div class="muted-2 mt-2" style="font-size:12px">
        ข้อมูลถูกเก็บใน browser ของเครื่องนี้ (localStorage) — ควรสำรองเป็นประจำ
      </div>
    </div>
  `;
});
...
document.addEventListener('submit', (e) => {
  if (e.target.id === 'companyForm') {
    e.preventDefault();
    const data = Object.fromEntries(new FormData(e.target).entries());
    Object.assign(DB.data.company, data);
    DB.save();
    ...
    const u = DB.data.auth.users.find(x => x.username === auth.currentUser.username);
    if (!u || u.password !== data.old) return toast('รหัสผ่านเดิมไม่ถูกต้อง', 'error');
    u.password = data.new;
    DB.save();
...
async function importBackup(event) {
  ...
  if (!file) return;
  const ok = await modal.confirm('นำเข้าข้อมูล', 'ข้อมูลปัจจุบันจะถูกแทนที่ด้วยข้อมูลจากไฟล์ ต้องการดำเนินการต่อ?');
  if (!ok) return;
  ...
  reader.onload = (e) => {
    try {
      ...
      toast('นำเข้าไม่สำเร็จ: ไฟล์ไม่ถูกต้อง', 'error');
    }
  };
...
async function resetData() {
  const ok = await modal.confirm('รีเซ็ตข้อมูล', 'จะลบข้อมูลทั้งหมดและกลับไปใช้ข้อมูลตัวอย่าง ต้องการดำเนินการต่อ?');
  if (!ok) return;
  DB.reset();
...
document.addEventListener('DOMContentLoaded', () => {
  // Sidebar nav
  $$('.nav-item').forEach(a => {
    a.addEventListener('click', (e) => { e.preventDefault(); router.go(a.dataset.page); });
  });
  // Mobile sidebar
